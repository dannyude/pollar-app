...
function LoginForm() {
  ...
  const handleLogin = () => {
    setSigning(true);
    login(); // Pollar opens its own sign-in; dev mode signs in immediately
  };

  // Pollar's sign-in resolves asynchronously, so move on once the session exists.
  useEffect(() => {
    if (user) router.replace(searchParams.get("redirect") || "/dashboard");
  }, [user, router, searchParams]);

  useEffect(() => {
    if (!user) setSigning(false);
  }, [user]);
      ...
      >
        {signing ? (
          ...
          <>
            ...
            {mode === "dev" ? "Continue (dev sign-in)" : "Continue with Pollar"} <FiArrowRight size={15} />
          </>
        )}
      </button>
