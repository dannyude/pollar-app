...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden relative">
      <Sidebar />
      <main className="flex-1 overflow-y-auto pb-20 pt-16 md:pb-0 md:pt-0" onScroll={handleScroll}>
        {/* Mobile Header */}
        <header className={`md:hidden fixed top-0 left-0 right-0 h-16 bg-white/90 backdrop-blur-md border-b border-surface-border px-5 flex items-center justify-between z-40 transition-transform duration-300 ${isHidden ? "-translate-y-full" : "translate-y-0"}`}>
